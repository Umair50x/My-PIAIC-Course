...
function makeAlbum(artist: string, title: string, tracks?: number) {
  const album = { artist, title, ...(tracks !== undefined ? { tracks } : {}) };
  return album;
}

const album1 = makeAlbum("Taylor Drift", "Full Space");
const album2 = makeAlbum("Dustin Geiber", "Daddy");
const album3 = makeAlbum("Rick Astley", "RickRoll");
const album4 = makeAlbum("Imran Khan", "Unstoppable", 10);
